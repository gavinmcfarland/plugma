...
const createDirectoryContents = (
	...
	packageManagerCommands?: { installCommand?: string; devCommand?: string; buildCommand?: string },
): void => {
	...
	filesToCreate.forEach((file) => {
		...
		if (stats.isFile()) {
			...
			const templateData: TemplateData = {
				...
				manifest,
				versions,
				installCommand: packageManagerCommands?.installCommand,
				devCommand: packageManagerCommands?.devCommand,
				buildCommand: packageManagerCommands?.buildCommand,
			};
			...
			createDirectoryContents(origFilePath, newDirPath, context, packageManagerCommands);
		}
	});
};
