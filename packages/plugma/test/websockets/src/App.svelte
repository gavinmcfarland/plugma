<script lang="ts">
	...
	function createMessage(content: string, to?: string, from?: string): Message {
		return {
	...
	function setupSocketListeners(socket: Socket) {
		...
		socket.on('FILE_CHANGED', (data) => {
			...
			sentMessages = [
				...sentMessages,
				createMessage(`File change notification sent to test-client: ${JSON.stringify(changeData)}`, 'test'),
			]
		})
		...
		socket.on('SEND_MESSAGE', (data) => {
			...
			receivedMessages = [...receivedMessages, createMessage(messageText, undefined, data.from)]
		})
	}
	...
	function sendMessage() {
		...
		sentMessages = [...sentMessages, createMessage(messageInput, selectedRoom)]
...
<main>
	...
	<div class="message-containers">
		<MessageList title="Sent Messages" messages={sentMessages} messageClass="sent-message" />
		<MessageList title="Received Messages" messages={receivedMessages} messageClass="received-message" />
	</div>
