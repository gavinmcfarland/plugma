...
import type { PlugmaRuntimeData, ShowUIOptions, WindowSettings } from '../../shared/types.js'
 ...
 */
export async function getWindowSettings(options?: Partial<ShowUIOptions>): Promise<WindowSettings> {
	...
	if (command === 'dev') {
		pluginWindowSettings = (await figma.clientStorage.getAsync(storageKeyDev)) as WindowSettings
		...
		pluginWindowSettings = (await figma.clientStorage.getAsync(storageKeyPreview)) as WindowSettings
