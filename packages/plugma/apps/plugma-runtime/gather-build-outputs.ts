...
const findFiles = (dir: string, base = ''): string[] => {
  debug('Finding files in directory:', dir);
      ...
      handler() {
        try {
          ...
          if (!fs.existsSync(sourcePath)) {
            console.error(`Source directory ${sourcePath} not found`);
            throw new Error('Build outputs missing - build may have failed');
          }

          debug('Gathering build outputs:', {
            sourcePath,
            targetPath,
            removeSourceDir,
          });
          ...
          for (const file of files) {
            ...
            if (!fs.existsSync(targetDir)) {
              debug('Creating target subdirectory:', targetDir);
