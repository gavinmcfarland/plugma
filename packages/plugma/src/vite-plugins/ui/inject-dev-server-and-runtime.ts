	...
	return {
		...
		transformIndexHtml(html: string): string {
			try {
				...
				const devServerAppProxyDev = readFileSync(join(__dirname, '../../apps/dev-server.html'), 'utf8')
          ...
          window.runtimeData = ${JSON.stringify(options)};
				...
				const transformedHtml = html.replace('<body>', `<body>${runtimeData}${devServerAppProxyDev}`)
