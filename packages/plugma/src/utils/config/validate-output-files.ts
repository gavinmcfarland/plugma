...
export async function cleanPluginOutputFiles(
	...
	const mainFilePath =
		currentFiles.manifest.main &&
		resolve(join(process.cwd(), currentFiles.manifest.main));
	const uiFilePath =
		currentFiles.manifest.ui &&
		resolve(join(process.cwd(), currentFiles.manifest.ui));
