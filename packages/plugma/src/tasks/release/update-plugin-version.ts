...
export class VersionUpdateError extends Error {
	constructor(
		message: string,
		public code: 'INVALID_VERSION' | 'INVALID_RELEASE_TYPE' | 'FILE_ERROR' | 'PARSE_ERROR',
	) {
		super(message);
...
export interface VersionUpdateOptions {
	...
	/** Type of release */
	type?: ReleaseType;
	/** Prefix to prepend to the version number */
	prefix?: string;
}
...
export interface VersionUpdateResult {
	...
	/** New version */
	newVersion: string;
	/** New version tag (with prefix if specified) */
	newTag: string;
 ...
 */
export async function versionUpdate(options: VersionUpdateOptions = {}): Promise<VersionUpdateResult> {
	...
	try {
		...
		if (options.version) {
			...
			if (!validateVersion(newVersion)) {
				throw new VersionUpdateError(`Generated invalid version: ${newVersion}`, 'INVALID_VERSION');
			}
		}
		...
		// Create tag with prefix (default to 'v' if no prefix provided)
		const tagPrefix = options.prefix || 'v';
		const newTag = options.prefix ? `${tagPrefix}@${newVersion}` : `${tagPrefix}${newVersion}`;
